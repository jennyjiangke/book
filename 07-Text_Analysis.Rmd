...
tweets_frequency<-tweets_frequency[!(tweets_frequency$words=="amp"),]
```
...
# Change the shape:
wordcloud2(tweets_frequency, size = 2.8, shape = 'star')
...
You can delete any nodes from the network. For example:

```{r}
# delete nodes from the network (optional)
mygraph <- delete_vertices(mygraph, "#blacklivesmatter")
```
...
threecore <- induced_subgraph(mygraph, kcore>=3)

#Centrality
degree_3 <- as.data.frame(degree(threecore,mode = "total"), normalized = TRUE)
eigen_3 <- as.data.frame(eigen_centrality(threecore), directed=FALSE)
between_3 <- as.data.frame(betweenness(threecore), directed=FALSE, normalized = TRUE)
...
library(SnowballC)
library(lsa)
c = cluster_edge_betweenness(threecore)
modularity(c)
```
...
Now, let's optimize the network visualization based on the cluster analysis

```{r}

library(ggraph)

ggraph(threecore, layout = "fr") +
  # Add nodes and edges
  geom_node_point() +
  geom_edge_link() +
  ...
  scale_edge_color_gradient(low = "grey", high = "black", guide = "legend") +
  scale_color_manual(values = c("blue", "red")) +
  labs(title = "A Semantic Network of #BlackLivesMatter", subtitle = "Based on Co-Occurrence")
